...
const ProjectCard  = ({ index, name, description, tags, image, live_website_link, source_code_link}) => {
  return(
    <motion.div variants={fadeIn("up", "spring", index * 0.5, 0.75)}>
      ...
      >
        <div className='relative w-full h-[230px]' data-carousel-item>
...
const Works = () => {
  ...
  // Chunk projects into arrays of three projects each
          ...
          {projectChunks.map((chunk, chunkIndex) => (
            <div key={`chunk-${chunkIndex}`} className="flex flex-wrap gap-6">
              {chunk.map((project, index) => (
                <ProjectCard key={`project-${index}`} index={index + (chunkIndex * 3)} {...project} className="h-full flex flex-wrap" />
              ))}
            </div>
          ))}
        </Carousel>
